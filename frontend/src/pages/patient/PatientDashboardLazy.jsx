...
export default function PatientDashboardLazy() {
  ...
  const toggleFavorite = async (pharmacyId) => {
    if (!userId) {
      toast.error(language === 'el' ? 'Ξ£Ο…Ξ½Ξ΄ΞµΞΈΞµΞ―Ο„Ξµ Ξ³ΞΉΞ± Ξ±Ο€ΞΏΞΈΞ®ΞΊΞµΟ…ΟƒΞ·' : 'Sign in to save');
      return;
    }
    ...
    try {
      if (isFavorite) {
        ...
        toast.success(language === 'el' ? 'Ξ‘Ο†Ξ±ΞΉΟΞ­ΞΈΞ·ΞΊΞµ Ξ±Ο€Ο Ξ±Ξ³Ξ±Ο€Ξ·ΞΌΞ­Ξ½Ξ±' : 'Removed from favorites');
      } else {
        await supabase
          .from('favorites')
        ...
        toast.success(language === 'el' ? 'Ξ ΟΞΏΟƒΟ„Ξ­ΞΈΞ·ΞΊΞµ ΟƒΟ„Ξ± Ξ±Ξ³Ξ±Ο€Ξ·ΞΌΞ­Ξ½Ξ±' : 'Added to favorites');
      }
    } catch (error) {
      ...
      if (error) {
        ...
        if (String(error.message).includes('CANCEL_LIMIT_REACHED')) {
          toast.error(language === 'el' 
            ? 'ΞΟ‡ΞµΞΉΟ‚ Ο†Ο„Ξ¬ΟƒΞµΞΉ Ο„ΞΏ ΟΟΞΉΞΏ Ξ±ΞΊΟ…ΟΟΟƒΞµΟ‰Ξ½ (3/ΞΌΞ®Ξ½Ξ±) Ξ³ΞΉΞ± Ξ±Ο€ΞΏΞ΄ΞµΞΊΟ„Ξ¬ Ξ±ΞΉΟ„Ξ®ΞΌΞ±Ο„Ξ±.'
            : 'You have reached the cancel limit (3/month) for accepted requests.');
        } else {
          toast.error(language === 'el' 
            ? 'Ξ‘Ο€ΞΏΟ„Ο…Ο‡Ξ―Ξ± Ξ±ΞΊΟΟΟ‰ΟƒΞ·Ο‚ Ξ±ΞΉΟ„Ξ®ΞΌΞ±Ο„ΞΏΟ‚.'
            : 'Failed to cancel request.');
        }
        return;
      }

      toast.success(language === 'el' ? 'Ξ¤ΞΏ Ξ±Ξ―Ο„Ξ·ΞΌΞ± Ξ±ΞΊΟ…ΟΟΞΈΞ·ΞΊΞµ.' : 'Request cancelled.');
      fetchMyRequests();
  ...
  const choosePharmacyForRequest = async (requestId, pharmacyId) => {
    ...
    try {
      ...
      toast.success(language === 'el'
        ? 'Ξ¤ΞΏ Ο†Ξ±ΟΞΌΞ±ΞΊΞµΞ―ΞΏ ΞµΟ€ΞΉΞ»Ξ­Ο‡ΞΈΞ·ΞΊΞµ.'
        : 'Pharmacy selected.');
      await fetchMyRequests();
    } finally {
  ...
  const durationOptions = [
    { value: '30m', label: language === 'el' ? '30 Ξ»ΞµΟ€Ο„Ξ¬' : '30 min' },
    { value: '1h', label: language === 'el' ? '1 ΟΟΞ±' : '1 hour' },
    { value: '2h', label: language === 'el' ? '2 ΟΟΞµΟ‚' : '2 hours' },
    { value: '3h', label: language === 'el' ? '3 ΟΟΞµΟ‚' : '3 hours' },
    { value: '5h', label: language === 'el' ? '5 ΟΟΞµΟ‚' : '5 hours' }
  ];
  ...
  const greeting = () => {
    ...
    if (language === 'el') {
      if (hour < 12) return 'ΞΞ±Ξ»Ξ·ΞΌΞ­ΟΞ±';
      if (hour < 18) return 'ΞΞ±Ξ»Ξ·ΟƒΟ€Ξ­ΟΞ±';
      return 'ΞΞ±Ξ»Ξ·ΟƒΟ€Ξ­ΟΞ±';
    }
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <Link to="/patient/reminders">
              <Button variant="ghost" size="sm" className="rounded-full gap-2 h-9" data-testid="nav-reminders-btn">
                ...
                {language === 'el' ? 'Ξ¥Ο€ΞµΞ½ΞΈΟ…ΞΌΞ―ΟƒΞµΞΉΟ‚' : 'Reminders'}
              </Button>
            </Link>
        ...
        <section className="page-enter">
          <div className="mb-5">
            ...
            <p className="text-pharma-slate-grey text-sm">
              {language === 'el' 
                ? 'Ξ¤ΞΉ Ο†Ξ¬ΟΞΌΞ±ΞΊΞΏ ΟΞ¬Ο‡Ξ½ΞµΟ„Ξµ ΟƒΞ®ΞΌΞµΟΞ±;'
              ...
              <div className="flex items-center gap-3">
                ...
                {!userLocation && (
                  ...
                  >
                    ...
                    {language === 'el' ? 'Ξ¤ΞΏΟ€ΞΏΞΈΞµΟƒΞ―Ξ±' : 'Location'}
                  </Button>
                )}
              </div>
      ...
      <Dialog open={requestDialogOpen} onOpenChange={setRequestDialogOpen}>
        <DialogContent className="bg-white rounded-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-pharma-charcoal">
                {language === 'el' ? 'Ξ•Ξ½ΞµΟΞ³Ο Ξ³ΞΉΞ±' : 'Active for'}
              </label>
