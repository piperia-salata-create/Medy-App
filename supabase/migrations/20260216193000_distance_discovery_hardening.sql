...
begin
  if exists (
    ...
    where n.nspname = 'public'
      and c.relname = 'public_pharmacies'
      and c.relkind = 'r'
  ) then
    execute '
      create index if not exists idx_public_pharmacies_lat_lng_not_null
      on public.public_pharmacies (latitude, longitude)
      where latitude is not null and longitude is not null
    ';
  end if;
end
...
begin
  ...
  -- Deterministic "my pharmacy" selection: latest created, tie-break by id.
  ...
  if v_actor_pharmacy_id is null or v_actor_lat is null or v_actor_lng is null then
    return;
  end if;
  ...
  with offers as (
    ...
    where eo.status = 'active'
      and eo.pharmacy_id <> v_actor_pharmacy_id
  ...
  select
    ...
    o.distance_km
  from offers o
  where p_nationwide or o.distance_km <= v_radius
  order by o.distance_km asc, o.created_at desc;
end;
$$;
...
begin
  ...
  -- Demand ownership follows existing app mapping (pharmacies.owner_id = auth.uid()).
  ...
  if v_demand_lat is null or v_demand_lng is null then
    return;
  end if;
